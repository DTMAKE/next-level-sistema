-- Add vendedor_id field to contratos table
ALTER TABLE public.contratos 
ADD COLUMN vendedor_id uuid REFERENCES public.profiles(user_id);

-- Add index for better performance
CREATE INDEX idx_contratos_vendedor_id ON public.contratos(vendedor_id);

-- Drop and recreate the trigger function to handle vendedor_id
...
BEGIN
    ...
    -- Use vendedor_id from contract if specified, otherwise use contract creator if they are a vendedor
    ...
    -- Create commission if we have a target vendedor
    IF target_vendedor_id IS NOT NULL THEN
        ...
        -- Only create commission if target is a vendedor
        IF vendedor_role = 'vendedor' THEN
            ...
            -- Create commission payable using contract date
            INSERT INTO public.transacoes_financeiras (
                user_id,
                tipo,
                data_transacao,
