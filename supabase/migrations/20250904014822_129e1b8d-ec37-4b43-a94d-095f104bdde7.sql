-- Fix the bug in generate_future_receivables_and_payables function
-- The data_transacao should use due_date (based on dia_vencimento) instead of current_month
...
BEGIN
    ...
    WHILE current_month <= end_month LOOP
        ...
        -- FIXED: Use due_date for data_transacao instead of current_month
        INSERT INTO public.transacoes_financeiras (
            ...
            'receita',
            due_date, -- FIXED: Changed from current_month to due_date
            due_date,
        ...
        IF contract_record.role = 'vendedor' AND admin_user_id IS NOT NULL THEN
            INSERT INTO public.transacoes_financeiras (
                ...
                due_date, -- FIXED: Changed from current_month to due_date
                due_date,
                'Comissão - Contrato ' || COALESCE(contract_record.numero_contrato, contract_record.id::text) || ' - ' || COALESCE(contract_record.vendedor_nome, 'Vendedor'),
                comissao_valor,
...
-- Update existing transactions to use the correct date
-- Fix existing transactions that have wrong dates
UPDATE public.transacoes_financeiras 
SET data_transacao = data_vencimento 
WHERE contrato_id IS NOT NULL 
AND tipo = 'receita' 
AND status = 'pendente'
AND data_transacao != data_vencimento;