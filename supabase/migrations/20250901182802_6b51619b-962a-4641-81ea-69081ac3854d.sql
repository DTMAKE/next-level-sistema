...
-- Clean up duplicate data more safely
-- First, delete financial transactions that reference duplicate commissions
...
WHERE comissao_id IN (
    ...
    AND EXISTS (
        SELECT 1 
        FROM public.comissoes c2 
        WHERE c2.vendedor_id = c.vendedor_id 
        AND c2.contrato_id = c.contrato_id 
        AND c2.mes_referencia = c.mes_referencia 
        AND c2.valor_comissao = c.valor_comissao
        AND c2.created_at < c.created_at
    )
);

-- Then delete duplicate commissions
DELETE FROM public.comissoes 
WHERE id IN (
    WITH duplicate_comissoes AS (
        SELECT 
            id,
            ROW_NUMBER() OVER (
                PARTITION BY vendedor_id, contrato_id, mes_referencia, valor_comissao 
                ORDER BY created_at ASC
            ) as rn
        FROM public.comissoes
        WHERE created_at > NOW() - INTERVAL '1 day'
    )
    SELECT id FROM duplicate_comissoes WHERE rn > 1
);

-- Clean up duplicate financial transactions
DELETE FROM public.transacoes_financeiras 
WHERE id IN (
    WITH duplicates AS (
        SELECT 
            id,
            ROW_NUMBER() OVER (
                PARTITION BY user_id, tipo, data_transacao, descricao, valor, categoria_id 
                ORDER BY created_at ASC
            ) as rn
        FROM public.transacoes_financeiras
        WHERE created_at > NOW() - INTERVAL '1 day'
        AND comissao_id IS NULL -- Only clean up non-commission related transactions to avoid FK issues
    )
