-- Desativar sistema antigo e corrigir duplicações de contas
...
AND EXISTS (
    ...
    AND tf2.descricao SIMILAR TO '%Contrato [^-]+ #[0-9]+ %'
    AND transacoes_financeiras.descricao SIMILAR TO '%' || 
        (SELECT COALESCE(c.numero_contrato, c.id::text) 
         FROM public.contratos c 
         WHERE c.id = tf2.contrato_id) || '%'
);
...
AND EXISTS (
    ...
    AND EXTRACT(MONTH FROM tf2.data_transacao) = EXTRACT(MONTH FROM transacoes_financeiras.data_transacao)
    AND EXTRACT(YEAR FROM tf2.data_transacao) = EXTRACT(YEAR FROM transacoes_financeiras.data_transacao)
);

-- 3. Atualizar trigger para evitar duplicações futuras
...
BEGIN
    ...
    WHILE current_month <= end_month LOOP
        ...
        AND DATE_TRUNC('month', data_transacao) = current_month;
        ...
        IF contract_record.role = 'vendedor' AND admin_user_id IS NOT NULL THEN
            ...
            JOIN public.comissoes c ON c.id = tf.comissao_id
            WHERE tf.tipo = 'despesa'
            AND c.contrato_id = p_contrato_id
            AND DATE_TRUNC('month', tf.data_transacao) = current_month;
...
-- 4. Adicionar índices únicos para prevenir duplicações futuras
CREATE UNIQUE INDEX IF NOT EXISTS idx_transacoes_contrato_mes_tipo 
ON public.transacoes_financeiras (contrato_id, date_trunc('month', data_transacao), tipo)
WHERE contrato_id IS NOT NULL;

-- 5. Executar limpeza final de órfãs
...
INSERT INTO public.audit_logs (
    ...
    'bulk_cleanup',
    jsonb_build_object(
        'message', 'Sistema de duplicação corrigido - contas órfãs removidas, trigger atualizado',
        'timestamp', now()
    )
);