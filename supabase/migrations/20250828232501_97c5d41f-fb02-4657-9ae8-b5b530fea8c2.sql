-- First, get the first admin user to own the global categories
DO $$
DECLARE
    admin_user_id uuid;
...
BEGIN
    ...
    IF admin_user_id IS NULL THEN
        RAISE NOTICE 'No admin user found, skipping global category creation';
        RETURN;
    END IF;
    
    -- Create or get global "Venda" category owned by first admin
    ...
    -- Create or get global "Comissões de Vendas" category owned by first admin
    ...
    -- Mark these categories as global by adding a comment or updating them
    UPDATE public.categorias_financeiras 
    SET user_id = admin_user_id
    WHERE id IN (categoria_venda_global_id, categoria_comissao_global_id);
END $$;

-- Update RLS policies to allow users to view these global categories
...
USING (
    ...
    (nome IN ('Venda', 'Comissões de Vendas')) -- Allow viewing global categories
);
...
BEGIN
    -- Get global category IDs (first ones created)
    SELECT id INTO categoria_venda_global_id
    FROM public.categorias_financeiras
    WHERE nome = 'Venda' AND tipo = 'receita'
    ORDER BY created_at ASC
    ...
    -- Only proceed if we found the global categories
    IF categoria_venda_global_id IS NOT NULL THEN
        ...
        FOR duplicate_record IN
            ...
            WHERE nome ILIKE '%venda%' AND tipo = 'receita' AND id != categoria_venda_global_id
        LOOP
    ...
    IF categoria_comissao_global_id IS NOT NULL THEN
        -- Update all transactions using duplicate "Comissões" categories to use global one
        FOR duplicate_record IN
            SELECT id FROM public.categorias_financeiras
            WHERE nome ILIKE '%comiss%' AND tipo = 'despesa' AND id != categoria_comissao_global_id
        LOOP
            UPDATE public.transacoes_financeiras
