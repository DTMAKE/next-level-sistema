-- Fix contract recurring transaction generation
...
DECLARE
    ...
    max_iterations integer := 60; -- Prevent infinite loops
BEGIN
    -- Get client name
    ...
    ELSE
        ...
        -- Fix: Ensure we have proper date handling
        current_month := DATE_TRUNC('month', NEW.data_inicio::timestamp);
        
        -- If data_fim is provided, use it; otherwise default to 2 years from start
        IF NEW.data_fim IS NOT NULL THEN
            end_month := DATE_TRUNC('month', NEW.data_fim::timestamp);
        ELSE
            end_month := DATE_TRUNC('month', NEW.data_inicio::timestamp + INTERVAL '2 years');
        END IF;
        
        RAISE LOG 'Creating recurring contract transactions from % to % for contract %', current_month, end_month, NEW.id;
        
        WHILE current_month <= end_month AND loop_count < max_iterations LOOP
            ...
            RAISE LOG 'Creating receivable for month % with due date %', current_month, due_date;
        ...
        -- Log if we hit the max iterations to prevent infinite loops
        IF loop_count >= max_iterations THEN
            RAISE WARNING 'Reached maximum iterations (%) when creating recurring transactions for contract %', max_iterations, NEW.id;
        END IF;
        
        RAISE LOG 'Created % monthly transactions for recurring contract %', loop_count, NEW.id;
    END IF;
