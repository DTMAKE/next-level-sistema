...
export default function Projetos() {
  ...
  const filteredProjetos = projetos.filter(projeto => projeto.nome.toLowerCase().includes(searchTerm.toLowerCase()) || projeto.descricao?.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    return <div className="space-y-3 lg:space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 min-w-0">
            <Button variant="outline" onClick={() => setSelectedProjeto("")} size="sm" className="self-start sm:self-auto flex-shrink-0">
            ...
            <div className="min-w-0">
              ...
              {projeto?.descricao && <p className="text-muted-foreground mt-1 text-sm sm:text-base line-clamp-2">{projeto.descricao}</p>}
            </div>
          </div>
        </div>
  ...
  return <div className="space-y-4 lg:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
        <div className="relative flex-1">
          ...
          <Input placeholder="Buscar projetos..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 text-sm sm:text-base" />
        </div>
      ...
      {isLoading ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
          ...
          {filteredProjetos.map(projeto => <Card key={projeto.id} className="projeto-card cursor-pointer" onClick={() => setSelectedProjeto(projeto.id)}>
              <CardHeader className="pb-3 sm:pb-6">
                ...
                backgroundColor: projeto.cor
                    ...
                    {projeto.privado && <Lock className="w-3 h-3 sm:w-4 sm:h-4 text-muted-foreground flex-shrink-0" />}
                  </CardTitle>
                  <div className="flex items-center gap-2">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={e => handleDeleteProject(projeto, e)} className="text-destructive">
                ...
                {projeto.descricao && <CardDescription className="text-xs sm:text-sm line-clamp-2">{projeto.descricao}</CardDescription>}
              </CardHeader>
              <CardContent className="pt-0">
      ...
      {filteredProjetos.length === 0 && !isLoading && <div className="text-center py-8 sm:py-12 px-4">
          ...
          <Button onClick={() => setShowProjetoDialog(true)} className="w-full sm:w-auto">
            <Plus className="w-4 h-4 mr-2" />
            Criar Primeiro Projeto
          </Button>
        </div>}

      <ProjetoDialog open={showProjetoDialog} onOpenChange={handleCloseProjetoDialog} projeto={editingProjeto} />

      <DeleteProjetoDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog} projeto={projectToDelete} />
    </div>;
}