...
import { Upload, FileText, Repeat } from "lucide-react";
import { format } from "date-fns";
import { useCreateContaReceber, CreateContaReceberData } from "@/hooks/useContasReceber";

const formSchema = z.object({
  descricao: z.string().min(1, "Nome da receita é obrigatório"),
...
export function ContaReceberDialog({ children }: ContaReceberDialogProps) {
  ...
  const onSubmit = async (data: FormData) => {
    const createData: CreateContaReceberData = {
    ...
    try {
      await createContaReceber.mutateAsync(createData);
      setOpen(false);
      form.reset();
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto z-[100] max-w-[95vw] sm:max-w-[600px]">
        <DialogHeader className="text-center pb-4">
          <DialogTitle className="text-2xl text-foreground">Nova Conta a Receber</DialogTitle>
          <DialogDescription className="text-muted-foreground">
            Preencha os dados abaixo para cadastrar a receita
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-base font-medium">Nome da Receita *</FormLabel>
                  <FormControl>
                    <Input 
                      placeholder="Ex: Prestação de Serviços, Venda, Consultoria..." 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-base font-medium">Data da Receita *</FormLabel>
                    <FormControl>
                      <Input 
                        type="date" 
                        className="h-12 text-base"
                  ...
                  <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                    ...
                    <div className="space-y-1 leading-none">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Gerar automaticamente esta receita todos os meses
                      </p>
                    </div>
                  </FormItem>
                )}
            ...
            <DialogFooter className="flex flex-col sm:flex-row gap-4 pt-6">
              ...
              <Button 
                type="submit" 
                disabled={createContaReceber.isPending}
                className="flex-1 h-12 text-base gradient-premium border-0 text-background font-medium"
              >
                {createContaReceber.isPending ? "Criando..." : "Criar Conta"}
              </Button>
            </DialogFooter>
          </form>
        </Form>
